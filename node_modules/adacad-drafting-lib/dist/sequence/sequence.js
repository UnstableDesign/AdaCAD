    ...
    */
     ...
     */
        ...
        import(row) {
            ...
            row.forEach(cell => {
                if (typeof cell == 'number')
                    ...
                    this.push((0, cell_1.cellToSequenceVal)(cell));
            });
            return this;
        }
        /**
       ...
       */
        pushRow(row) {
            row.forEach(cell => {
                if (typeof cell == 'number')
                    ...
                    const val = (0, cell_1.getCellValue)(cell);
                    if (val === null)
                        this.push(2);
                    else
                        this.push(val ? 1 : 0);
        ...
        deleteAndDrawIn(val) {
            if (val < 1 || val >= this.state.length)
                return this;
            const deleted = this.state[val];
        ...
        matchSize(seq) {
            if (this.state.length < seq.length()) {
                const diff = seq.length() - this.state.length;
        ...
        computeFilter(filter, seq) {
            ...
            for (let i = 0; i < this.length(); i++) {
                ...
                if (seq.get(i) == 1)
                    bool_b = true;
                const res = (0, utils_1.computeFilter)(filter, bool_a, bool_b);
                switch (res) {
                    case null:
                        this.state[i] = 2;
                        break;
    ...
    class TwoD {
         ...
         */
        mapToSystems(weftsys, warpsys, weft_system_map, warp_system_map, ends, pics) {
            //create a blank draft of the size needed that we'll copy into 
            const mapped_seq = new TwoD().setBlank(2).fill(ends, pics);
            //now map the new values within that space
            let within_sequence_i = 0;
            let within_sequence_j = 0;
            for (let i = 0; i < pics; i++) {
                const active_weft_system = weft_system_map.get(i % weft_system_map.length());
                if (weftsys.find(el => el == active_weft_system) !== undefined) {
                    within_sequence_j = 0;
                    for (let j = 0; j < ends; j++) {
                        const active_warp_system = warp_system_map.get(j % warp_system_map.length());
                        if (warpsys.length == 0) {
         ...
         * @returns
         */
        mapToWarpSystems(warpsys, weft_system_map, warp_system_map, ends, pics) {
            //create a blank draft of the size needed that we'll copy into 
            const mapped_seq = new TwoD().setBlank(2).fill(ends, pics);
            //now map the new values within that space
            let within_sequence_i = 0;
         ...
         * @returns
         */
        mapToWeftSystems(weftsys, weft_system_map, warp_system_map, ends, pics) {
            //create a blank draft of the size needed that we'll copy into 
            const mapped_seq = new TwoD().setBlank(2).fill(ends, pics);
            //now map the new values within that space
            let within_sequence_i = 0;
        ...
        overlay(seq, consider_heddle_down_as_unset) {
            ...
            let total_wefts = 0;
            total_wefts = (0, utils_1.lcm)([this.wefts(), seq.wefts()], utils_2.defaults.lcm_timeout);
            if (total_wefts !== -1)
                total_wefts = Math.max(this.wefts(), seq.wefts());
            let total_warps = 0;
            total_warps = (0, utils_1.lcm)([this.warps(), seq.warps()], utils_2.defaults.lcm_timeout);
            if (total_warps !== -1)
                total_warps = Math.max(this.warps(), seq.warps());
       ...
       */
         ...
         * @returns
         */
        placeInLayerStack(cur_warp_sys, warp_sys_above, cur_weft_sys, weft_sys_above, weft_system_map, warp_system_map) {
                ...
                cur_weft_sys.forEach(cur_weft => {
                    for (let i = 0; i < this.state.length; i++) {
                        for (let j = 0; j < this.state[0].length; j++) {
                            const weft_sys = weft_system_map.get(i % weft_system_map.length());
                            const warp_sys = warp_system_map.get(j % warp_system_map.length());
        ...
        pushWarpSequence(seq) {
            ...
            if (height > 0 && height != seq.length) {
                let lcm_val = (0, utils_1.lcm)([height, seq.length], utils_2.defaults.lcm_timeout);
                if (lcm_val !== -1)
                    lcm_val = Math.max(height, seq.length);
                ...
                for (let j = 0; j < width; j++) {
                    const col = this.state.map(el => el[j]);
                    const col_seq = new OneD(col).resize(lcm_val).val();
                    for (let i = 0; i < lcm_val; i++) {
       ...
       */
        pushWeftSequence(seq) {
            if (this.state.length > 0 && this.state[0].length !== seq.length) {
                const width = this.state[0].length;
                let lcm_val = (0, utils_1.lcm)([width, seq.length], utils_2.defaults.lcm_timeout);
                if (lcm_val !== -1)
                    lcm_val = Math.max(width, seq.length);
        ...
        setBlank(val = 2) {
            const res = new OneD().push(val).val();
            this.state = [res];
            return this;
        }
        wefts() {
        ...
        fill(w, h) {
            ...
            if (this.state.length == 0)
                return this;
            const len_h = this.state.length;
            for (let i = 0; i < h; i++) {
                let row;
                const len_w = this.state[i % len_h].length;
                if (w >= len_w) {
        ...
        import(dd) {
            ...
            dd.forEach((row, i) => {
                this.state.push([]);
                row.forEach((cell, j) => {
                    this.state[i][j] = (0, cell_1.cellToSequenceVal)(cell);
                });
            });
            return this;
        }
        ...
        export() {
            ...
            this.state.forEach((row, i) => {
                ...
                row.forEach((cell_val, j) => {
                    dd[i][j] = (0, cell_1.createCellFromSequenceVal)(cell_val);
                });
            });
            return dd;
        }
