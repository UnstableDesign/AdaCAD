...
export class NoteComponent implements OnInit {
  ...
  setPosition(topleft: Point) {
    ...
    if (note_container) note_container.style.transform = 'none'; //negate angulars default positioning mechanism
    if (note_container) note_container.style.top = this.topleft.y + "px";
    if (note_container) note_container.style.left = this.topleft.x + "px";
  }
  ...
  dragMove($event: CdkDragMove) {
    ...
    let op_topleft_inscale = {
      x: note_container?.offsetLeft ?? 0,
      y: note_container?.offsetTop ?? 0
    }
  ...
  dragEnd($event: CdkDragEnd) {
    ...
    if (op_container) op_container.style.transform = 'none'; //negate angulars default positioning mechanism
    if (op_container) {
