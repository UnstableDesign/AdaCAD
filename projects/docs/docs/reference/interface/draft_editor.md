...
<!-- TODO (screenshot): img/draft_editor_overview.jpeg is from AdaCAD 4 and is out of date. The sidebar is now a four-panel accordion (Edit, Edit Loom Settings, Information, New / Import) and the selection actions have moved from a pop-up on the draft into the sidebar. Re-capture and re-letter to match the sections below. -->
...
The **Edit** panel holds everything you use to design upon the draft.
...
<!-- TODO (screenshot): img/draft_editor_designmodes.jpeg still illustrates the concept correctly, but it labels the toggle "drafting mode" where the UI now says "Drafting Style". Re-capture the label when convenient. -->
...
<!-- TODO (screenshot): img/draft_editor_select.jpeg and img/draft_editor_select.gif are from AdaCAD 4 and show a pink circular pop-up button on the draft that no longer exists. Selection actions now live in the sidebar's Select panel. Re-capture both. -->
