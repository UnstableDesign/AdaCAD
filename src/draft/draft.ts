  ...
  if (params.drawdown !== undefined) {
    ...
    d.drawdown.forEach((row, i) => {
      row.forEach((cell, j) => {
        ...
        if (val !== undefined) cell = setCellValue(cell, getCellValue(val));
      })
    })
  }
 ...
 */
export const wefts = (d: Drawdown | Array<Array<boolean>>): number => {
 ...
 */
export const warps = (d: Drawdown | Array<Array<boolean>>): number => {
  if (d === null || d == undefined) return 0;
 ...
 */
export const getHeddle = (d: Drawdown, i: number, j: number): boolean | null => {
...
export const pasteIntoDrawdown = (
  ...
  const rows = wefts(fill_pattern);
  const cols = warps(fill_pattern);
...
export const getDraftAsImage = (draft: Draft, pix_per_cell: number, floats: boolean, use_color: boolean, mats: Array<Material>): ImageData => {
  ...
  for (let i = 0; i < wefts(draft.drawdown); i++) {
    for (let j = 0; j < warps(draft.drawdown); j++) {
      ...
      const cell_val = getCellValue(draft.drawdown[i][j]);
      ...
      const weft_mat: Material | undefined = mats.find(el => el.id == draft.rowShuttleMapping[i]);
      const warp_mat: Material | undefined = mats.find(el => el.id == draft.colShuttleMapping[j]);
  ...
  const image = new ImageData(uint8c, warps(draft.drawdown) * pix_per_cell);
  return image;


}

export const drawDraftViewCell = (arr: Uint8ClampedArray, i: number, j: number, val: boolean | null, dim: number, warp_num: number, use_color: boolean, warp: DraftCellColor, weft: DraftCellColor): Uint8ClampedArray => {
  let color_id = 0;

  const cols = rendering_color_defaults.concat([warp, weft])
...
const drawFloatViewCell = (arr: Uint8ClampedArray, i: number, j: number, val: boolean | null, dim: number, warp_num: number, use_color: boolean, warp: DraftCellColor, weft: DraftCellColor): Uint8ClampedArray => {
  let color_id = 0;

  const bg: DraftCellColor = {
    id: 'background',
    r: 245,
    g: 245,
    b: 245,
...
export const exportDrawdownToArray = (drawdown: Drawdown): Array<number> => {
  ...
  for (let i = 0; i < wefts(drawdown); i++) {
    for (let j = 0; j < warps(drawdown); j++) {
      const val = getCellValue(drawdown[i][j]);
      switch (val) {
        case null:
          arr.push(2);
          break;
 ...
 */
export const generateMappingFromPattern = (drawdown: Drawdown, pattern: Array<number>, type: string): Array<number> => {
...
export const flipDraft = (d: Draft, horiz: boolean, vert: boolean): Promise<Draft> => {
  ...
  if (horiz) {
    ...
    for (let j = 0; j < warps(d.drawdown); j++) {
      const flipped_j = warps(d.drawdown) - 1 - j;
