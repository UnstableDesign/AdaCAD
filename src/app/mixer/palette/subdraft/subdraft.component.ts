...
export class SubdraftComponent implements OnInit, OnChanges {
  ...
  /**when the scale is changed, triggers this function */
  @Input() 
  set scale(value: number) {
    this._scale = value;
    console.log("set rescale", value);

    this.rescale(value);
  }

 /** if the input draft is marked dirty, triggers call to clean */
  @Input() 
  set dirty(value: boolean) {
    console.log("setting dirty", value);
    if(value) this.drawDraft();
  }
  ...
  */
  ...
  ngOnInit(){
    ...
    else  this.interlacement = utilInstance.resolvePointToAbsoluteNdx(this.bounds.topleft, this._scale);

    const draft = this.tree.getDraft(this.id);

    if(draft !== undefined){
      this.bounds.width = draft.warps * this._scale;
      this.bounds.height = draft.wefts * this._scale;
    }else{
      this.bounds.width = draft.warps * this._scale;
      this.bounds.height = draft.wefts * this._scale;
    }
  ...
  rescale(scale:number){
    ...
    //redraw at scale
    const container: HTMLElement = document.getElementById('scale-'+draft.id);
    ...
    this.bounds.width = draft.warps * scale;
    this.bounds.height = draft.wefts * scale;
   ...
   */
   rescaleForBitmap(){
    ...
    // this.bounds.topleft = {
    //   x: this.interlacement.j * this._scale,
    //   y: this.interlacement.i * this._scale
    // };

    // this.bounds.width = this.draft.warps * this._scale;
    // this.bounds.height = this.draft.wefts * this._scale;
 ...
 */
 public resolveNdxToPoint(ndx:Interlacement) : Point{
  ...
  let y = this.bounds.topleft.y + ndx.i * this._scale;
  let x = this.bounds.topleft.x + ndx.j * this._scale;
  return {x: x, y:y};
 ...
 */
  public resolvePointToNdx(p:Point) : Interlacement{
    ...
    let i = Math.floor((p.y -this.bounds.topleft.y) / this._scale);
    let j = Math.floor((p.x - this.bounds.topleft.x) / this._scale);
 ...
 */
  ...
  //   this.bounds.width = temp.warps * this._scale;
  //   this.bounds.height = temp.wefts * this._scale;
  ...
  async designActionChange(e){
    ...
    switch(e){
      ...
      default: 
        // const drafts = await this.ops.getOp(e).perform([draft], []);
        // drafts.forEach(draft => {
        //   this.tree.setDraft(this.id, draft);
        //   this.drawDraft();
        // });
