import { Cell, getCellValue, Draft, wefts, warps, Drawdown, SystemList, getDraftName } from "../draft";
import { Loom, LoomSettings } from "../loom";
import { MaterialMap } from "../material";
import { OperationInlet, OpInput, OpParamVal, OpParamValType, OpInletValType } from "../operations";
...
export const getAllDraftsAtInletByLabel = (op_inputs: Array<OpInput>, label: string): Array<Draft> => {

  if (!operationHasInputs(op_inputs) || label === '') return [];
  else {
    ...
    op_inputs.forEach((input, ndx) => {
      ...
      const found = input.inlet_params.findIndex(p => label.includes(<string>p));
