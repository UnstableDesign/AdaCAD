...
const perform = (param_vals: Array<OpParamVal>) => {
  ...
  const size: number = <number>getOpParamValById(0, param_vals);
  let selection: number = <number>getOpParamValById(1, param_vals);
...
interface ComboNode {
  parent: ComboNode | null;
  value: number,
  set: Array<Array<number>>,
  children: Array<ComboNode>;
}

let cur_set: { warps: number, wefts: number } = { warps: 0, wefts: 0 };
...
function addRow(draft: Draft, tree: ComboTree, i: number, n: number) {
  ...
  opts.forEach(opt => {
    ...
    expanded_drafts.push(initDraftWithParams({ warps: n, wefts: n, drawdown: pattern.slice() }));
...
function getOptions(seq: Array<number>, tree: ComboTree): Array<Array<number>> {
  ...
  let node: ComboNode | null | undefined = null;


  seq.forEach(val => {
    node = children.find(el => el.value == val);
    if (node === undefined) children = [];
    else if (node == null) children = [];
    else children = node.children.slice() ?? [];
  });

  if (node === undefined || node === null) return [];
  else return (node as ComboNode).set.slice();
}
...
function getAllPossible(n: number): Promise<Array<Array<number>>> {
  ...
  let all_combos: Array<Array<number>> = [];
