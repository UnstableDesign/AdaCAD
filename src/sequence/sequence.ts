...
*/
    ...
    resize(n: number): OneD {
      ...
      if (this.state.length == 0) return this;
 ...
 */
   ...
   */
    ...
    import(row: Array<Cell> | Array<number>) {
      ...
      row.forEach(cell => {
        ...
        else this.push(cellToSequenceVal(cell));
      })
      return this;
   ...
   */
    ...
    computeFilter(filter: string, seq: OneD) {
      ...
      for (let i = 0; i < this.length(); i++) {
        ...
        const res: boolean | null = computeFilter(filter, bool_a, bool_b);
 ...
 */
  ...
  export class TwoD {
     ...
     */
    mapToSystems(weftsys: Array<number>, warpsys: Array<number>, weft_system_map: OneD, warp_system_map: OneD, ends: number, pics: number) {

      //create a blank draft of the size needed that we'll copy into 
      const mapped_seq: TwoD = new TwoD().setBlank(2).fill(ends, pics);
     ...
     * @returns 
     */
    mapToWarpSystems(warpsys: Array<number>, weft_system_map: OneD, warp_system_map: OneD, ends: number, pics: number) {
     ...
     * @returns 
     */
    mapToWeftSystems(weftsys: Array<number>, weft_system_map: OneD, warp_system_map: OneD, ends: number, pics: number) {
     ...
     * @returns 
     */
    overlay(seq: TwoD, consider_heddle_down_as_unset: boolean) {
    ...
    setUnsetOnWeft(i: number, val: number) {
      ...
      const weft: Array<number> = this.getWeft(i);
      weft.forEach((el, j) => {
   ...
   */
    setUnsetOnWarp(j: number, val: number) {
      ...
      const warp: Array<number> = this.getWarp(j);
      warp.forEach((el, i) => {
        if (el == 2) {
     ...
     * @returns 
     */
    placeInLayerStack(cur_warp_sys: Array<number>, warp_sys_above: Array<number>, cur_weft_sys: Array<number>, weft_sys_above: Array<number>, weft_system_map: OneD, warp_system_map: OneD) {
   ...
   */
    ...
    setBlank(val: number | boolean = 2): TwoD {
    ...
    import(dd: Drawdown) {
      ...
      dd.forEach((row, i) => {
        this.state.push([]);
        row.forEach((cell, j) => {
          this.state[i][j] = cellToSequenceVal(cell);
        })
      });
      return this;
    }
